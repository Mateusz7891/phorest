...
def send_to_someone_else_other_test(playwright: Playwright) -> None:
    ...
    gift_card = GiftCardBuilder().with_send_to_someone_else_data_other().build()

    footer = Footer(page)

    buy_gift_page = BuyGiftCardPage(page)
    buy_gift_page.fill_gift_page_send_to_someone_else_other(gift_card)
...
with sync_playwright() as playwright:
    send_to_someone_else_other_test(playwright)